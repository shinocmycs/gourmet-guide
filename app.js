...
async function cloudUploadAll(){
  ...
  if(!confirm(`${restaurants.length}店舗をクラウドへ移行します。\n写真も順番にアップロードします。開始しますか？`))return;
  let ok=0,ng=0;
...
$('editorForm').onsubmit=async e=>{
  ...
  const obj={
    id,name:$('name').value.trim(),address:$('address').value.trim(),phone:$('phone').value.trim(),
    closedDays:$('closedDays').value.trim(),tabelogUrl:$('tabelogUrl').value.trim(),rank:+$('rank').value,
    visitFrequency:$('visitFrequency').value,genre:$('genre').value,price:$('price').value,note:$('note').value.trim(),
    favorite:$('favorite').checked,personalRanking:$('personalRanking').value?+$('personalRanking').value:null,
    location:(currentEditLocation?.addressSource===$('address').value.trim()?currentEditLocation:null),
    ...
    thumbnail:editingPhotos[0]?await makeThumb(editingPhotos[0]):null,photoOptimizedV17:true,
    hours:{lunch:{enabled:$('lunchEnabled').checked,open:$('lunchOpen').value,close:$('lunchClose').value},
  ...
  if(cloudGroup){
    try{
      ...
      alert('端末には保存しましたが、クラウド保存に失敗しました。\n通信状態を確認してもう一度編集・保存してください。');
...
};if('serviceWorker'in navigator){navigator.serviceWorker.register('./sw.js?v=2000').then(r=>r.update()).catch(()=>{});navigator.serviceWorker.addEventListener('controllerchange',()=>{if(!sessionStorage.getItem('gg-sw-reloaded')){sessionStorage.setItem('gg-sw-reloaded','1');location.reload();}})}});
