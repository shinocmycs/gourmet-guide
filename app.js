...
async function refresh(){restaurants=await loadAll();render()}function render(){const q=$('search').value.trim().toLowerCase(),genreFilter=$('genreFilter')?.value||'';let list=restaurants.filter(r=>[r.name,r.address,r.genre,r.note].join(' ').toLowerCase().includes(q));if(genreFilter)list=list.filter(r=>r.genre===genreFilter);if(currentFilter==='favorite')list=list.filter(r=>r.favorite);if(currentFilter==='ranked')list=list.filter(r=>r.personalRanking).sort((a,b)=>a.personalRanking-b.personalRanking);if(currentFilter==='near')list=list.filter(r=>r.location&&currentLocation&&hav(currentLocation,r.location)<=5).sort((a,b)=>hav(currentLocation,a.location)-hav(currentLocation,b.location));$('countLabel').textContent=(q||genreFilter||currentFilter!=='all')?`表示：${list.length}件 / 全${restaurants.length}件`:`登録店舗：${restaurants.length}件`;$('list').innerHTML=list.map(r=>`<article class="card" data-id="${r.id}">${r.photos?.[0]?`<img class="thumb" src="${r.photos[0]}">`:'<div class="thumb placeholder">🍴</div>'}<div><h3>${esc(r.name)}</h3><div class="stars">${stars(r.rank)}</div><div class="meta">${esc(r.genre||'ジャンル未設定')}${r.price?' ・ '+esc(r.price):''}${dist(r)?' ・ '+dist(r):''}</div><div class="meta">${esc(r.address||'住所未設定')}</div><div class="badges">${r.favorite?'<span class="badge">♥ お気に入り</span>':''}<span class="badge">${esc(r.visitFrequency||'未訪問')}</span>${r.personalRanking?`<span class="badge">🏆 ${r.personalRanking}位</span>`:''}</div></div></article>`).join('');$('empty').classList.toggle('hidden',restaurants.length!==0);document.querySelectorAll('.card').forEach(c=>c.onclick=()=>showDetail(c.dataset.id))}
...
async function listDatabaseNames(){
  ...
  return ['gourmetGuideDB','GourmetGuideDB','gourmet-guide','restaurants'];
}
function readStoreFromDatabase(dbName,storeName){
  return new Promise((resolve)=>{
    let req;
    try{req=indexedDB.open(dbName)}catch{return resolve([])}
    req.onerror=()=>resolve([]);
    req.onsuccess=()=>{
      const d=req.result;
      if(!d.objectStoreNames.contains(storeName)){d.close();return resolve([])}
      try{
        const t=d.transaction(storeName,'readonly');
        const s=t.objectStore(storeName);
        const r=s.getAll();
        r.onsuccess=()=>{const v=r.result||[];d.close();resolve(v)};
        r.onerror=()=>{d.close();resolve([])};
      }catch{d.close();resolve([])}
    };
  });
}
async function recoverLegacyData(){
  const names=[...new Set(await listDatabaseNames())];
  const candidates=[];
  for(const n of names){
    for(const store of ['restaurants','restaurant','stores']){
      const rows=await readStoreFromDatabase(n,store);
      if(rows.length)candidates.push({db:n,store,rows});
    }
  }
  if(!candidates.length){
    alert('このアプリ領域では旧データを見つけられませんでした。\nバックアップJSONがある場合は「復元」を使ってください。');
    return;
  }
  candidates.sort((a,b)=>b.rows.length-a.rows.length);
  const best=candidates[0];
  const rows=best.rows.filter(r=>r&&r.id&&r.name);
  if(!rows.length){
    alert('データベースは見つかりましたが、店舗データを読み取れませんでした。');
    return;
  }
  if(!confirm(`${rows.length}件の旧店舗データを見つけました。\n現在のデータに取り込みますか？`))return;
  for(const r of rows)await put(r);
  await refresh();
  alert(`${rows.length}件を復旧しました。すぐに「バックアップ」を押して保存してください。`);
}
