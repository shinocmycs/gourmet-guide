...
if($('useLocationBtn'))$('useLocationBtn').onclick=async()=>{
  const address=$('address')?.value.trim()||'';
  if(!address){alert('先に店舗住所を入力してください。\n現在地を登録する場合も、どの住所の店舗か分かるよう住所入力が必要です。');return;}
  try{
    if(typeof photoBusy==='function')photoBusy(true,'現在地を取得しています…');
    const pos=await getLoc();
    currentEditLocation={
      lat:Number(pos.lat),
      lon:Number(pos.lon),
      addressSource:address,
      geocodeProvider:'device-current-location',
...
経度 ${pos.lon.toFixed(6)}`);
 ...
 if(ng)alert(`${ok}枚を読み込みました。\n${ng}枚はクラウドから取得できませんでした。\n取得できない写真は一度iPhoneの「写真」へ保存してから選択してください。`);
  ...
  if(address&&!locationMatchesAddress(currentEditLocation,address)){
    try{
      if(typeof photoBusy==='function')photoBusy(true,'保存住所から位置を取得しています…');
  ...
  await put({
    ...
    closedDays:$('closedDays').value.trim(),
    tabelogUrl:$('tabelogUrl').value.trim(),
    rank:+$('rank').value,
    visitFrequency:$('visitFrequency').value,
    genre:$('genre').value,
    price:$('price').value,
